...
TIME_STEP=30;INPUT_SIZE=70;CELL_SIZE=38;PFC_LearningRate=0.001;OUT_SIZE=CELL_SIZE
...
PFC_cell = tf.contrib.rnn.BasicLSTMCell(num_units=CELL_SIZE)
...
PFC_in_=np.zeros([BATCH_SIZE,TIME_STEP,70])
imgn_im = np.zeros([BATCH_SIZE, TIME_STEP, 784])
inp_Edr = np.zeros([BATCH_SIZE, TIME_STEP, 64])
imgn_Edr = np.zeros([BATCH_SIZE, TIME_STEP, 32])
...
for ep in range (tot_episodes):#episode
    ...
    for tt in range(TIME_STEP):
        if tt<cmd_posi:
            inp_im = b_x_1
        else:
            inp_im= b_x_2*0

        en3_, ff1_ = sess.run([en3, ff1],{tf_x: inp_im, ph_encoded: ph_encoded_, ph_switch: ph_switch_, ph_dis_e: ph_dis_e_})
        aud = np.tile(cmd_bi[tt], [BATCH_SIZE, 1])
        PFC_in_[:, tt, :]=np.concatenate((aud,en3_,ff1_),axis=1)

    _, cost_his[ep] = sess.run([PFC_train, PFC_loss],
                               {PFC_x: PFC_in_[:, 0:TIME_STEP - 1, :], PFC_y: PFC_in_[:, 1:TIME_STEP, :OUT_SIZE]})
    ...
    if ((ep+1) % 1000 == 0):
        ...
        for tt in range(TIME_STEP):
            ...
            en3_, ff1_ = sess.run([en3, ff1],
                                  {tf_x: inp_im, ph_encoded: ph_encoded_, ph_switch: ph_switch_, ph_dis_e: ph_dis_e_})
            aud = np.tile(cmd_bi[tt], [BATCH_SIZE, 1])
            PFC_in_[:, tt, :] = np.concatenate((aud, en3_, ff1_), axis=1)
        ...
        file_nm = '/Users/fengqi/Pycharm_py36/QF/Lng_LSTM' + str(ep) + str(np.random.random(1)) + '.jpeg'
        plt.savefig(file_nm)

        try:

            saver = tf.train.Saver()
            path = '/Users/fengqi/Pycharm_py36/QF/Lng_LSTM' + str(ep+1) + '/'
            file_='CKPT001'
            os.mkdir(path)
