...
for ep in range (tot_episodes):#episode
    ...
    _,cost_his[ep]=sess.run([PFC_train,PFC_loss],{PFC_x: PFC_in[:,1:tot_t_steps-1,:],PFC_y:PFC_in[:,2:tot_t_steps,:]})
    ...
    if ((ep+1) % 1000 == 0):
        try:
            ...
            path = '/Users/fengqi/Pycharm_py36/QF/LSTM' + str(ep+1) + '/'
            file_='CKPT001'
            os.mkdir(path)
...
saver = tf.train.Saver()
saver.save(sess, '/Users/fengqi/Pycharm_py36/QF/_PFC_LSTM_10000')
...
file_nm='/Users/fengqi/Pycharm_py36/QF/_LSTM'+str(ep)+str(np.random.random(1))+'.jpeg'
