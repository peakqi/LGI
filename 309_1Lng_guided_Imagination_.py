...
def affine_transform_x(b_x,dx):
    ...
    seq = iaa.Sequential([iaa.Affine( translate_percent={"x": dx/28})])
...
with tf.variable_scope('dlPFC'):
    ...
    dlPFC_loss = tf.losses.mean_squared_error(labels=dlPFC_y, predictions=dlPFC_outs) #+30*tf.losses.mean_squared_error(labels=dlPFC_y[:,25,:], predictions=dlPFC_outs[:,25,:])  # compute cost
...
path = '/Users/fengqi/Pycharm_py36/QF/dlPFC' + str(chk_num) + '_0/'
file_='CKPT001'
...
img_orig=np.zeros([TIME_STEP-1,784])
img_pred=np.zeros([TIME_STEP-1,784])
...
for ep in range (1):#episode
    ...
    b_x, b_y = mnist.test.next_batch(BATCH_SIZE)

    b_x_1=affine_transform_init(np.reshape(b_x,[BATCH_SIZE,784]))

    cmd,del_x=Gen_lng()
    cmd_bi=sentence_to_vec(cmd)
    b_x_2 = affine_transform_x(b_x_1, del_x)
    ...
    for ii in range(BATCH_SIZE):
        aud_[ii]=cmd_bi  #cmd_bi (30,6)    aud_[128,30,6]
    ips_out_=sess.run(IPS_outs,{IPS_x:aud_}) #ips_out_ (128, 30, 1)

    for ii in range(TIME_STEP-1):
        if ii<=cmd_posi:
            img_orig[ii]=b_x_1[text_ind]
            whole_[:,ii,6:38]=en3_1
            whole_[:,ii,38:70]=ff1_1
        else:
            img_orig[ii] = b_x_2[text_ind]
            whole_[:, ii, 6:38] = en3_2
            whole_[:, ii, 38:70] = ff1_2
    ...
    for ii in range(29):
        img_pred[ii]=sess.run(decoded, {dcd_in:np.reshape(dlPFC_outs_[0,ii,6:38],[1,32])})

        aud_pred=aud_code_to_txt(np.round(dlPFC_outs_[0,ii,0:6]))
        a[0][ii].imshow(np.reshape(img_orig[ii], [28, 28]))
        a[0][ii].set_xticks(());a[0][ii].set_yticks(());
        a[1][ii].set_xticks(());
        a[1][ii].imshow(np.reshape(img_pred[ii], [28, 28]))
        a[1][ii].set_yticks(());
        a[0][ii].set_title(cmd[ii])
        a[1][ii].set_title(aud_pred)
    rand_1=np.random.random(1)
    file_nm = '/Users/fengqi/Pycharm_py36/QF/' + str(chk_num) + str(rand_1) + '.jpeg'
    plt.savefig(file_nm)
    ...
    for ii in range(3):
        img_pred[ii+24]=sess.run(decoded, {dcd_in:np.reshape(dlPFC_outs_[0,24+ii,6:38],[1,32])})
        a[0][ii].imshow(np.reshape(img_orig[24+ii], [28, 28]))
        a[0][ii].set_xticks(());a[0][ii].set_yticks(());
        a[1][ii].set_xticks(());
        a[1][ii].imshow(np.reshape(img_pred[24+ii], [28, 28]))
        a[1][ii].set_yticks(());
    a[0][ii].set_title(cmd)
    file_nm = '/Users/fengqi/Pycharm_py36/QF/0' + str(chk_num) + str(rand_1) + '_short.jpeg'
    plt.savefig(file_nm)
