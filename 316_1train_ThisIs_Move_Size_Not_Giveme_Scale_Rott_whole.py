...
path = '/Users/fengqi/Pycharm_py36/QF/ThisMoveSizeNotGiveScaleRott' + str(chk_num) + '/'
file_='CKPT001'
...
for ep in range (tot_episodes):#episode
    ...
    if (ep % 100 == 0):
        ...
        np.save('ThisMoveSizeNotGiveScaleRott_whole_cost', cost_his)

    if ((ep+1) % 1000 == 0):

        try:
            saver = tf.train.Saver()
            path = '/Users/fengqi/Pycharm_py36/QF/ThisMoveSizeNotGiveScaleRott_whole' + str(ep+1) + '/'
            file_='CKPT001'
            os.mkdir(path)
