...
PFC_lstm_kernel=np.load('/Users/fengqi/Pycharm_py36/QF/PFC_lstm_kernel.npy')
PFC_lstm_bias=np.load('/Users/fengqi/Pycharm_py36/QF/PFC_lstm_bias.npy')
PFC_dense_kernel=np.load('/Users/fengqi/Pycharm_py36/QF/PFC_dense_kernel.npy')
PFC_dense_bias=np.load('/Users/fengqi/Pycharm_py36/QF/PFC_dense_bias.npy')
...
en3_ph = tf.placeholder(tf.float32, [None,  32])
ff1_eval = tf.layers.dense(en3_ph, n_encoded, tf.nn.sigmoid,kernel_initializer=tf.constant_initializer(wem),bias_initializer=tf.constant_initializer(bem),trainable=False)
...
weights_lstm_w2 = tf.get_default_graph().get_tensor_by_name('dlPFC/rnn/basic_lstm_cell/kernel:0')
weights_lstm_b2 = tf.get_default_graph().get_tensor_by_name('dlPFC/rnn/basic_lstm_cell/bias:0')
sess.run(tf.assign(weights_lstm_w2, PFC_lstm_kernel))
sess.run(tf.assign(weights_lstm_b2, PFC_lstm_bias))

weights_lstm_w3 = tf.get_default_graph().get_tensor_by_name('dlPFC/dense/kernel:0')
weights_lstm_b3 = tf.get_default_graph().get_tensor_by_name('dlPFC/dense/bias:0')
sess.run(tf.assign(weights_lstm_w3, PFC_dense_kernel))
sess.run(tf.assign(weights_lstm_b3, PFC_dense_bias))
...
en3_last=np.zeros([BATCH_SIZE,n_l3])
ff1_last=np.zeros([BATCH_SIZE,n_encoded])
b_x, b_y = mnist.train.next_batch(BATCH_SIZE)
b_x_1 = b_x * 0
cmd_list=['give me a 9','rotate 180','this is ','enlarge','the size is ']
cmd_len=len(cmd_list)
f, a = plt.subplots(cmd_len, 29)

for ep in range (cmd_len):#episode
    ...
    if ep==0:
        ...
        en3_1=en3_last;
        ff1_1=sess.run(ff1_eval,{en3_ph:np.reshape(en3_last,[BATCH_SIZE,32])})
    ...
    ind_show=BATCH_SIZE - 1
    for ii in range(29):
        img_pred[ind_show, ii] = sess.run(decoded,{dcd_in: np.reshape(dlPFC_outs_[ind_show, ii, 6:38], [1, 32])})
        aud_pred = aud_code_to_txt(np.round(dlPFC_outs_[ind_show, ii, 0:6]))
        a[ep][ii].imshow(np.reshape(img_pred[ind_show, ii], [28, 28]))
        a[ep][ii].set_title(aud_pred)
        a[ep][ii].set_xticks(());
        a[ep][ii].set_yticks(());
    en3_last=dlPFC_outs_[:,int(cmd_sz1-1),6:38]

rand_1 = np.random.random(1)
file_nm = '/Users/fengqi/Pycharm_py36/QF/_imagination_ATL'  + str(rand_1) + '.jpeg'
fig = plt.gcf()
