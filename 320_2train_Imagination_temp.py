...
for ep in range (tot_episodes):#episode
    ...
    rand_2=np.random.random(1)
    if rand_2>0.5:
        rand_0 =2#size 6#rott180
    elif rand_2>0.3 and rand_2<=0.5:
        rand_0=5#scale
    elif rand_2>0.2 and rand_2<=0.3:
        rand_0=4#giveme
    elif rand_2<=0.2 and rand_2>0.1:
        rand_0=1#this is
    elif rand_2<=0.1:
        rand_0=6#rott180
    ...
    if (ep % 100 == 0):
        ...
        np.save('_Imagination_this_cost', cost_his)

    if ((ep+1) % 1000 == 0):

        try:
            saver = tf.train.Saver()
            path = '/Users/fengqi/Pycharm_py36/QF/_Imagination_this' + str(ep+1) + '/'
            file_='CKPT001'
            os.mkdir(path)
