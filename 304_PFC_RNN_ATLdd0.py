...
de0 = tf.layers.dense(dcd_x, n_d0, tf.nn.sigmoid,kernel_initializer=tf.constant_initializer(wd0),bias_initializer=tf.constant_initializer(bd0),trainable=False)
...
for ep in range (tot_episodes):#episode
    ...
    for time_step in range(tot_t_steps):
        ...
        srsvrv_[:, 3] =0
        PFC_in[:,time_step,:]=sess.run (PFC_IN, {tf_x: im})



    _,cost_his[ep]=sess.run([PFC_train,PFC_loss],{PFC_x: PFC_in[:,1:tot_t_steps-1,:],PFC_y:PFC_in[:,2:tot_t_steps,:32]})
    ...
    if ((ep+1) % 1000 == 0):
        try:
            ...
            path = '/Users/fengqi/Pycharm_py36/QF/RNN' + str(ep+1) + '/'
            file_='CKPT001'
            os.mkdir(path)
...
saver = tf.train.Saver()
saver.save(sess, '/Users/fengqi/Pycharm_py36/QF/_PFC_RNN_10000')
...
file_nm='/Users/fengqi/Pycharm_py36/QF/_RNN'+str(ep)+str(np.random.random(1))+'.jpeg'
plt.savefig(file_nm)
