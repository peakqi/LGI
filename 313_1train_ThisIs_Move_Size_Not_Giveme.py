...
path = '/Users/fengqi/Pycharm_py36/QF/ThisMoveSizeNot' + str(chk_num) + '/'
file_='CKPT001'
...
for ep in range (tot_episodes):#episode
    ...
    rand_0 = np.random.randint(5)  # rand_0>0.5 move;  <0.5 thisis
    ...
    if (ep % 100 == 0):
        ...
        np.save('ThisMoveSizeNotGive_cost', cost_his)

    if ((ep+1) % 1000 == 0):

        try:
            saver = tf.train.Saver()
            path = '/Users/fengqi/Pycharm_py36/QF/ThisMoveSizeNotGive' + str(ep+1) + '/'
            file_='CKPT001'
            os.mkdir(path)
