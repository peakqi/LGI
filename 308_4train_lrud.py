...
def affine_transform_xy(b_x, d,lrud):
    d3=lrud
    sz1, sz2 = b_x.shape
    ...
    seq = iaa.Sequential([iaa.Affine(translate_percent={"x": xx/56,"y": yy/56})])
...
with tf.variable_scope('dlPFC'):
    ...
    dlPFC_loss = tf.losses.mean_squared_error(labels=dlPFC_y, predictions=dlPFC_outs)+lambda1*tf.losses.mean_squared_error(labels=dlPFC_y[:,25,:], predictions=dlPFC_outs[:,25,:])  # compute cost
    dlPFC_loss1 = tf.losses.mean_squared_error(labels=dlPFC_y[:,25,:], predictions=dlPFC_outs[:,25,:])
...
for ep in range (tot_episodes):#episode
    ...
    cmd,del_x,lrud,cmd_sz=Gen_lng()
    cmd_bi=sentence_to_vec(cmd)
    b_x_2 = affine_transform_xy(b_x_1, del_x,lrud)
    ...
    _,cost_his[ep],cost1=sess.run([dlPFC_train,dlPFC_loss,dlPFC_loss1],{dlPFC_x:whole_[:,0:29,:],dlPFC_y:whole_[:,1:30,0:38]})
    if (ep % 100 == 0):
        # aa=whole_[:, 1:30, 0:38]
        # bb = sess.run(dlPFC_outs,{dlPFC_x: whole_[:, 0:29, :],})
        print('ep_step:', ep, '| train loss: %.8f' % cost_his[ep],'| train loss_pred_bit: %.8f' % cost1)
        np.save('lrud_cost', cost_his)
        # cc=aa[1] - bb[1]
        # print(cc[28,1:5])
    ...
    if ((ep+1) % 1000 == 0):
        ...
        try:
            ...
            path = '/Users/fengqi/Pycharm_py36/QF/lrud' + str(ep+1) +'_Lambda'+str(lambda1)+ '/'
            file_='CKPT001'
            os.mkdir(path)
