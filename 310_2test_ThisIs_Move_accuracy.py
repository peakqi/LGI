...
path = '/Users/fengqi/Pycharm_py36/QF/ThisMove' + str(chk_num) + '/'
file_='CKPT001'
...
count_right=0
count_wrong=0
for ep in range (128):#episode
    #prep inp [en3_32,ATL32,aud6,num1] total 71
    rand_0 = 0#np.random.random(1)  # rand_0>0.5 move;  <0.5 thisis
    ...
    for ii in range(29):
        ...
        img_pred[0,ii]=sess.run(decoded, {dcd_in:np.reshape(dlPFC_outs_[0,ii,6:38],[1,32])})
        aud_pred=aud_code_to_txt(np.round(dlPFC_outs_[0,ii,0:6]))
        # a[0][ii].imshow(np.reshape(img_orig[0,ii], [28, 28]))
        # a[0][ii].set_xticks(());a[0][ii].set_yticks(());
        # a[1][ii].set_xticks(());
        # a[1][ii].imshow(np.reshape(img_pred[0,ii], [28, 28]))
        # a[1][ii].set_yticks(());
        # a[0][ii].set_title(aud_code_to_txt(aud_[0,ii]))
        # a[1][ii].set_title(aud_pred)
        if ii==8:
            aa1=aud_code_to_txt(aud_[0,ii])
        elif ii==7:
            aa2=aud_pred
    if aa1==aa2:
    ...
    # file_nm = '/Users/fengqi/Pycharm_py36/QF/ZMoveThis/MoveThis' + str(chk_num) + str(rand_1) + '.jpeg'
    # fig = plt.gcf()
