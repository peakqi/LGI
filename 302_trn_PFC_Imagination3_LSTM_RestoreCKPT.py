...
file_addr='/Users/fengqi/Pycharm_py36/QF/LSTM'+str(eps_num)+'/CKPT001'
...
for ep in range (1):#episode
    ...
    for time_step in range(tot_t_steps):
        if time_step > 1:
            im_dcd_=sess.run(decoded,{dcd_x:np.reshape(outs_[:,time_step-2,:],[128,32])})
            a[1][time_step].imshow(np.reshape(im_dcd_[2], [28, 28]))
        else:
            a[1][time_step].imshow(np.zeros( [28, 28]))
file_nm='/Users/fengqi/Pycharm_py36/QF/_LSTM'+str(eps_num)+str(np.random.random(1))+'.jpeg'
