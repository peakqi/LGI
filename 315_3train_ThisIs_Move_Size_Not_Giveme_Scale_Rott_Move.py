...
path = '/Users/fengqi/Pycharm_py36/QF/ThisMoveSizeNotGiveScaleRott_whole' + str(chk_num) + '/'
file_='CKPT001'
...
for ep in range (tot_episodes):#episode
    ...
    if np.random.random(1)>0.5:
        rand_0 = 0#
    # elif np.random.random(1)>0.5:
    #     rand_0=6
    else:
        rand_0=np.random.randint(7)
    ...
    for ii in range(BATCH_SIZE):
        if rand_0 == 0:
            ...
            cmd, del_x[ii], cmd_sz[ii] = Gen_lng_rott()
    ...
    for ii in range(BATCH_SIZE):
        ...
        if rand_0 == 0:
            ...
            b_x_2[ii] = affine_transform_r(np.reshape(b_x_1[ii], [1, 784]), del_x[ii])
    ...
    for jj in range(BATCH_SIZE):
        for ii in range(TIME_STEP):
            ...
            if rand_0 == 0 or rand_0 == 4 or rand_0 == 5 or rand_0==6:
    ...
    if (ep % 100 == 0):
        ...
        np.save('ThisMoveSizeNotGiveScaleRottMove_whole_cost', cost_his)

    if ((ep+1) % 1000 == 0):

        try:
            saver = tf.train.Saver()
            path = '/Users/fengqi/Pycharm_py36/QF/ThisMoveSizeNotGiveScaleRottMove_whole' + str(ep+1) + '/'
            file_='CKPT001'
            os.mkdir(path)
